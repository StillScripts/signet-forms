...
- [x] **[TASK-47] Replace Laravel starter home page** - Rebuilt `welcome.blade.php` as a branded Signet Forms landing page with header, hero, six-feature grid, guest-only sign-up banner, and footer. Auth-aware: authenticated visitors see a "Go to dashboard" CTA routed to their current team (falls back to tenant registration if they have none); guests see login and register actions. Switched from the starter's inlined CSS to the project's Tailwind v4 pipeline via Vite. Expanded the home page test to cover render, guest CTAs, and the auth-aware dashboard link.

- [x] **[TASK-25] Save and resume** - Add `resume_token` (64 chars, unique), `resume_token_expires_at`, `resume_page_index`, and `is_draft` to submissions. PublicFormPage gains `saveProgress()` / `loadDraft()`; multi-page wizard resumes at saved step via Filament `startOnStep()`. Queued `SubmissionResumeLink` mailable with 30-day expiry. Save endpoint rate-limited per IP+email (5/min, 20/hr); `respondent_email` protected against override; submit and saveProgress both guard on `is_draft`. Drafts excluded from admin list, bulk export, and submission count badges. Patient Intake seeder template expanded to a 4-page flow via new `multiPageSchema()` helper. 24 new tests.
