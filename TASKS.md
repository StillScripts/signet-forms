# Tasks

## Active

- [ ] **[TASK-2] Add Projects to the application** - Introduce the Project model as an organizational layer between Teams and Forms. Teams own Projects, Projects will eventually own Forms. Includes model, migration, factory, Filament resource, policy, and tests.

## Waiting On

## Someday

## Done

- [x] **[TASK-1] Set up feature documentation workflow** - Add docs/ directory structure, integrate doc-coauthoring, product-brainstorming, and write-spec skills into the development workflow so every feature gets a spec with clear reasoning. Eventually these docs will power a doc site.
